...
public class MemberServiceImp implements MemberService{
	...
	public MemberVO updateMember(MemberVO input, MemberVO user) {
		...
		if(input.getMe_address1() == null || input.getMe_address1().length() == 0) {
			input.setMe_address1(user.getMe_address1());
		}
		if(input.getMe_address2() == null || input.getMe_address2().length() == 0) {
			input.setMe_address2(user.getMe_address2());
		}
		if(input.getMe_address3() == null || input.getMe_address3().length() == 0) {
			input.setMe_address3(user.getMe_address3());
		}
	...
	public String findPw(MemberVO member) {
		...
		byte[] bytes = null;
		try {
			bytes = Files.readAllBytes(Paths.get("D:\\JAVA_KNH\\java_knh\\spring_port\\src\\main\\webapp\\resources\\template.html"));
